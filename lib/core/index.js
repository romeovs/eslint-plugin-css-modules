...
export const getAST = (filePath: string, syntax: ?string): gASTNode | null => {
  const fileContent = fs.readFileSync(filePath);

  const ast = gonzales.parse(fileContent.toString(), {
    syntax: syntax || path.extname(filePath).slice(1),
  });
